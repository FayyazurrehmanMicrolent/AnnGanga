...
export default function HamperProductsPage() {
  ...
  useEffect(() => {
    const fetchHamperProducts = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/product?tag=hamper');
        ...
        if (data.status === 200) {
          ...
          if (hamperProducts.length === 0) {
            setError('No gift hamper products available at the moment.');
          }
        } else {
          throw new Error(data.message || 'Failed to fetch hamper products');
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">     
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Gift Hampers</h1>
          <p className="text-gray-600">
            Perfect gifts for your loved ones ({products.length} products)
          </p>
        </div>
        ...
        {products.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">No gift hamper products available at the moment.</p>
          </div>
        ) : (
