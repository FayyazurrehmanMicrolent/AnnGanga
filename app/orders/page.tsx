...
import { Package, Truck, CheckCircle, XCircle, Clock, MapPin, Phone, Calendar, AlertCircle } from 'lucide-react';
...
export default function OrdersPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (s === 'delivered') return { icon: CheckCircle, color: 'bg-emerald-700 text-emerald-100 border-emerald-300', label: 'Delivered' };
    if (s === 'dispatched') return { icon: Truck, color: 'bg-purple-700 text-purple-100 border-purple-300', label: 'On the way' };
    if (s === 'confirmed' || s === 'packed') return { icon: Package, color: 'bg-blue-700 text-blue-100 border-blue-300', label: 'Processing' };
    if (s === 'cancelled') return { icon: XCircle, color: 'bg-red-700 text-red-100 border-red-300', label: 'Cancelled' };
    return { icon: Clock, color: 'bg-yellow-700 text-amber-100 border-amber-300', label: 'Pending' };
  };

  const canCancel = (status: string) => !['dispatched', 'delivered', 'cancelled'].includes(status.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 py-8 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        {orders.length === 0 ? (
          /* Empty State */
          <div className="bg-white rounded-3xl shadow-2xl p-16 text-center border border-emerald-100">
            ...
            <p className="text-gray-600 text-lg mb-10">Time to fill your kitchen!</p>
            <Link href="/">
              <button className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-10 py-5 rounded-2xl text-xl shadow-lg hover:shadow-xl transition">
                Start Shopping
              </button>
            </Link>
          </div>
          ...
          <div className="space-y-8">
            {orders.map((order) => {
              ...
              return (
                <div key={order.orderId} className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-200 hover:shadow-2xl transition-all duration-300">
                  ...
                  <div className="bg-gradient-to-r from-emerald-600 to-green-600 text-white p-5">
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-xl font-bold tracking-wider">{order.orderId}</p>
                      </div>
                      <div className="text-right">
                        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold border-2 ${status.color}`}>
                          ...
                          {order.items.map((item, i) => (
                            <div key={i} className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl border border-gray-200">
                              <div>
                                ...
                                {item.weightOption && <p className="text-sm text-emerald-600">{item.weightOption}</p>}
                                <p className="text-sm text-gray-600">Qty: {item.quantity}</p>
                              </div>
                              <p className="text-xl font-bold text-emerald-600">₹{item.total}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-5">
                        ...
                        <div className="bg-gray-50 p-5 rounded-2xl border border-gray-300">
                          ...
                          <div className="space-y-2 text-sm">
                            ...
                            {order.discount > 0 && <div className="flex justify-between text-emerald-600 font-medium"><span>Discount</span> <span>-₹{order.discount}</span></div>}
                            {order.rewardDiscount > 0 && <div className="flex justify-between text-emerald-600 font-medium"><span>Rewards Used</span> <span>-₹{order.rewardDiscount}</span></div>}
                            <div className="flex justify-between"><span>Delivery</span> <span className={order.deliveryCharges === 0 ? 'text-emerald-600' : ''}>₹{order.deliveryCharges}</span></div>
                            <div className="border-t-2 border-emerald-200 pt-3 flex justify-between font-bold text-lg">
                        ...
                        {/* Payment & Delivery */}
                        <div className="text-sm space-y-2 bg-emerald-50 p-4 rounded-2xl border border-emerald-200">
                        ...
                        {order.trackingId && (
                          <div className="bg-blue-50 p-4 rounded-2xl border border-blue-200 text-center">
                            <p className="font-medium text-blue-800">Tracking ID: {order.trackingId}</p>
                            {order.trackingUrl && (
                              <a href={order.trackingUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-sm font-medium">
                                Track Package →
                              </a>
                            )}
                          </div>
                    ...
                    <div className="mt-8 pt-6 border-t-2 border-gray-100 flex gap-4">
                      {canCancel(order.orderStatus) && (
                        <button
                          ...
                          className="flex-1 py-4 border-2 border-red-500 text-red-600 rounded-2xl font-bold hover:bg-red-50 transition"
                        >
                          Cancel Order
                        </button>
                      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 border border-gray-200">
            ...
            <p className="text-gray-700 mb-6">
              Order <span className="font-mono font-bold text-red-600">{selectedOrder.orderId}</span> will be cancelled.
            </p>
            <textarea
              placeholder="Reason for cancellation (required)"
              value={cancelReason}
              onChange={(e) => setCancelReason(e.target.value)}
              className="w-full border-2 border-gray-300 rounded-2xl p-4 focus:border-red-500 focus:outline-none resize-none"
              rows={4}
            />
            <div className="flex gap-4 mt-6">
              <button
                onClick={() => { setSelectedOrder(null); setCancelReason(''); }}
                className="flex-1 py-4 border-2 border-gray-300 text-gray-800 rounded-2xl font-bold hover:bg-gray-50"
              >
                Back
              </button>
              <button
                ...
                className="flex-1 py-4 bg-red-600 text-white rounded-2xl font-bold hover:bg-red-700 disabled:bg-gray-400 transition shadow-lg"
              >
